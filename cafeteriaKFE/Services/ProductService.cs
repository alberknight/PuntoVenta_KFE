...
public sealed class ProductService : IProductService
{
    ...
    public async Task<IdentityResult> CreateAsync(CreateProductResponse dto)
    {
        ...
        if (await _repo.ExistsBarCodeAsync(dto.BarCode))
        {
            return IdentityResult.Failed(
                new IdentityError { Description = "Ya existe un producto con ese c�digo de barras." }
            );
        }
    ...
    public async Task<IdentityResult> UpdateAsync(UpdateProductResponse dto)
    {
        ...
        {
            return IdentityResult.Failed(
                new IdentityError { Description = "Ya existe otro producto con ese c�digo de barras." }
            );
        }
    ...
    {
        if (id <= 0)
            return IdentityResult.Failed(new IdentityError { Description = "Id inv�lido." });
